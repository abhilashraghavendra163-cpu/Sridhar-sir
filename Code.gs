  ...
  Object.values(SHEET_NAMES).forEach(name => {
    ...
    if (!sheet) {
      ...
      if (name === SHEET_NAMES.SUMMARY) {
        headers = ['Date', 'Opening_Balance', 'Total_Deposits', 'Total_Expenses', 'Closing_Balance', 'Verified', 'Variance', 'Remarks', 'Counted_By', 'Verified_By'];
...
function doPost(e) {
  try {
    ...
    if (action === 'getUsers') {
      ...
      return jsonResponse({ summaries, txs, denoms });
...
function deleteTransaction(id) {
  ...
  const data = sheet.getDataRange().getValues();
  const headers = data[0];
  const idColIndex = headers.indexOf('ID'); // Note JS side must send unique ID when saving
...
function drawDailySheet(dateStr) {
  ...
  sheet.getRange(6, 1, 1, 4).setFontWeight("bold").setBackground("#F8FAFC");
